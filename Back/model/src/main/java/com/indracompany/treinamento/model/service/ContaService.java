...
public class ContaService extends GenericCrudService<Conta, Long, ContaRepository>{
	...
	public void depositar (DepositoDTO depositar) {
		Conta contas = this.consultarConta(depositar.getAgencia(), depositar.getNumeroConta());
		contas.setSaldo(depositar.getValor() + contas.getSaldo());
		super.salvar(contas);
		Operacao operacao= new Operacao();
		operacao.setConta(contas);
		operacao.setTpOperacao("D".charAt(0));
		operacao.setValor(depositar.getValor());
